...
Local FastAPI backend, SQLite (WAL), mobile PWA UI, APScheduler worker.
...
Telegram variables are prefixed with `SMART_FRIDGE_` (see `backend/app/config.py`).
...
**Observability:** Set `SMART_FRIDGE_LOG_LEVEL=INFO` (default) to see per-request **`[trace=uuid]`** correlation and specialist pipeline **stage timings** (`timing_ms` in scan responses / DB). Use `DEBUG` for more verbose third-party logs.
...
This repo is intentionally **barcode/QR-only**. Product names and expiry dates are confirmed by the user.
